...
namespace CMS.Savings.Maintenance.Model
{
    class InterestRatesModel
    {
        ...
        public int checkBracketed(int savings, double MinBal, int interestId)
        {
            DAL dal = new DAL(ConfigurationManager.ConnectionStrings["CMS"].ConnectionString);
            String sql = "SELECT COUNT(*) FROM INTEREST_RATE WHERE InterestId != @interestId  AND MaximumRange != 0 AND AccountTypeId = @savings AND Status = 1 AND (MaximumRange >= @MinBal OR MinimumRange >= @MinBal)";
            Dictionary<String, Object> parameters = new Dictionary<string, object>();
            parameters.Add("@interestId", interestId);
            parameters.Add("@savings", savings);
            parameters.Add("@MinBal", MinBal);
        ...
        {
            DAL dal = new DAL(ConfigurationManager.ConnectionStrings["CMS"].ConnectionString);
            String sql = "SELECT COUNT(*) FROM INTEREST_RATE WHERE InterestId != @interestId AND AccountTypeId = @savings AND Status = 1 AND MaximumRange = 0 AND MinimumRange <= @MinBal";
            Dictionary<String, Object> parameters = new Dictionary<string, object>();
            parameters.Add("@interestId", interestId);
            parameters.Add("@savings", savings);
            parameters.Add("@MinBal", MinBal);
        ...
        {
            DAL dal = new DAL(ConfigurationManager.ConnectionStrings["CMS"].ConnectionString);
            String sql = "SELECT i.InterestId AS 'Interest Id', i.AccountTypeId AS 'Account Type Id', SavingsTypeName AS 'Account Type', CONCAT(InterestRate, '% ', Per) AS 'Interest Rate', CONCAT(MinimumRange, '-', MaximumRange) AS 'Balance Range', i.Status, isArchived, i.DateCreated AS 'Date Created', i.DateModified AS 'Last Modified' FROM INTEREST_RATE i INNER JOIN SAVINGS_ACCOUNT_TYPE s ON i.AccountTypeId = s.AccountTypeId WHERE isArchived = 0 AND SavingsTypeName LIKE(@searchName)";
