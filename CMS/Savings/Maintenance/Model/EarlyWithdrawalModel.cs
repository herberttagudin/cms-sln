...
namespace CMS.Savings.Maintenance.Model
{
    class EarlyWithdrawalModel
    {
        ...
        public int checkBalanceRange(double MinBal, double MaxBal, int savings, int id)
        {
            DAL dal = new DAL(ConfigurationManager.ConnectionStrings["CMS"].ConnectionString);
            String sql = "SELECT COUNT(*) FROM EARLY_WITHDRAWAL WHERE AccountTypeId = @savings AND EarlyWithdrawalId != @id AND MaximumRange != 0 AND Status = 1 AND ((@MinBal BETWEEN MinimumRange AND MaximumRange) OR (@MaxBal BETWEEN MinimumRange AND MaximumRange) OR (MinimumRange >= @MinBal AND MaximumRange <= @MaxBal))";
            Dictionary<String, Object> parameters = new Dictionary<string, object>();
            parameters.Add("@savings", savings);
            parameters.Add("@id", id);
        ...
        public int checkUnbracketed(int savings, double MinBal, int earlyWithdrawalId)
        {
            DAL dal = new DAL(ConfigurationManager.ConnectionStrings["CMS"].ConnectionString);
            String sql = "SELECT COUNT(*) FROM EARLY_WITHDRAWAL WHERE EarlyWithdrawalId != @earlyWithdrawalId AND AccountTypeId = @savings AND Status = 1 AND MaximumRange = 0  AND MinimumRange <= @MinBal";
            Dictionary<String, Object> parameters = new Dictionary<string, object>();
            parameters.Add("@earlyWithdrawalId", earlyWithdrawalId);
            parameters.Add("@savings", savings);
            parameters.Add("@MinBal", MinBal);
